...
class Ion_auth_model extends MY_Model
{
	...
	public function delete($restaurant_id){ 
		$sql = "DELETE auth_tokens FROM auth_tokens LEFT JOIN restaurant_users on restaurant_users.user_id=auth_tokens.user_id WHERE (auth_tokens.android_device_token IS NOT NULL OR auth_tokens.ios_device_token IS NOT NULL OR auth_tokens.web_device_token IS NOT NULL) AND auth_tokens.app_type = ".CART_FROM_POS." AND restaurant_users.restaurant_id = ".$restaurant_id;
		$this->db->query($sql);
	}
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE || 1)
		{
			// store the new password and reset the remember code so all remembered instances have to re-login
	 ...
	 * @author Mathew
	 **/
	public function identity_check($identity = '', $userGroup=null)
	{
		$this->trigger_events('identity_check');
		if (empty($identity)){ return FALSE; }
		$this->db->where($this->identity_column, $identity);
		if($userGroup == CUSTOMER_GROUP_ID){ 
			$this->db->where('group_id', $userGroup);	
		}else{
			$this->db->where('group_id!='.CUSTOMER_GROUP_ID,null,false);	
		}
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array() )
	{
		...
		if ($this->identity_check($identity, $groups[0]))
		{
			
			$error['error']= $this->set_error($this->lang->line("email_already_exist"));
		}
		elseif ( !$this->config->item('default_group', 'ion_auth') && empty($groups) )
		{
			
			$error['error']= $this->set_error('Account creation missing default group');
		}
		...
		if( !isset($query->id) && empty($groups) )
		{
			...
			$error['error']= $this->set_error('Account creation invalid default group');
		}
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $acceptedUserGroup=[])
	{
		...
		$this->db->select($this->identity_column . ', email, id, password, status,is_email_verified, last_login');
                $this->db->group_start();
		$this->db->where($this->identity_column, $identity);
                if( is_numeric($identity) ) {
                    $this->db->or_where("phone_no", $identity);
                }
                $this->db->group_end();
		if(!empty($acceptedUserGroup) && is_array($acceptedUserGroup)){
                    $this->db->where_in('group_id', $acceptedUserGroup);
		}
     ...
     * the configuration item recheck_timer. If it does, then it will check if the user is still status
     * @return bool
     */
