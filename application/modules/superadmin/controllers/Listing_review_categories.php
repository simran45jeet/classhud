...
class Listing_review_categories extends MY_Controller 
{
    public $layout_view = "layout/".SUPERADMIN;
    private $type = REVIEW_CATEGORIES_TYPES_LISTING;
    /**
     * Construct
     */
    public function __construct() {
        ...
        $this->data['title'] = $this->lang->line("heading_listing_review_category_title");
    }

    public function index($page_no=1){
        $post_data = $this->get_data;
        $post_data["all_records"]=true;
        $records = $this->review_categories->get_records($post_data,true,$page_no);    
        $count = $records['count'];
    ...
    public function add(){
        ...
        $this->data['title'] = $this->lang->line("heading_add_listing_review_category_title");
        ...
        if( !empty($this->post_data) ) {
            ...
            if( $this->form_validation->run() ) {
                ...
                $blog_id = $this->review_categories->insert($insert_data);
                if( !empty($blog_id) ) {
    ...
    public function edit($encoded_id){
        ...
        $record = $this->review_categories->get_record($edited_id)["record"];
        ...
        if( !empty($this->post_data) ) {
            ...
            if( $this->form_validation->run() ) {
                ...
                $blog_id = $this->review_categories->update($insert_data,$edited_id);
                if( !empty($blog_id) ) {
